...
const DetailView = () => {
  ...
  useEffect(() => {
    ...
    if (id !== null) {
      const destination = Detailed.destinations.find((dest) => dest.id === id);
      setData(destination);
    }
  }, []);
    ...
    <section className="flex flex-col mt-20 items-center justify-center w-full h-full">
      {data && (
        <div className="flex flex-col items-center  justify-center w-11/12 h-full">
          ...
          <p className="text-lg sm:text-lg md:text-xl mt-5 text-justify">
            {data.description}
          </p>

          <div
            className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-4  gap-5 gap-y-4 mt-5 w-full"
            style={{ alignItems: "flex-start" }}
          >
            <div className="flex flex-col p-4">
              <h2 className="text-lg sm:text-lg md:text-xl font-bold">
                Category
              </h2>
              <p className="text-lg sm:text-lg md:text-xl mt-2">
                {data.category}
              </p>
            </div>
            <div className="flex flex-col  p-4">
            ...
            <div className="flex flex-col  p-4">
              ...
              <ul className="list-disc text-justify">
                {data.popularActivities.map((activity, index) => (
                  <li
                    key={index}
                    className="text-lg sm:text-lg md:text-xl mt-2"
                  >
                    {activity}
                  </li>
                ))}
